...
def generate_data(init_num, final_num, xtrain, ytrain, args_train, netG, pos_coeff, device):
    x_train = xtrain.copy()
    # reshape (size, 1,len) to (size, len)
    ...
    for i, j in enumerate(init_num):
        sample_batch = int(final_num - j)
        classidx = [i] * sample_batch
        y_train = np.hstack([y_train, np.array(classidx)])
        ...
        fake_data = np.array(fake_sample.to('cpu'))
        fake_data = fake_data.reshape(sample_batch, args_train.feature_len)
        x_train = np.vstack([x_train, fake_data])

    data = np.hstack([x_train, y_train[:, None]])
    np.random.shuffle(data)
    x_train = data[:, :-1]
    y_train = data[:, -1]
