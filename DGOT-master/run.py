...
# %%
def train(args, ds, init_num, final_num):
    ...
    if args.class_num > 2:
        from scripts.evaluate_multi import sample_evaluate, generate_data
    else:
        from scripts.evaluate_binary import sample_evaluate, generate_data
    ...
    with tqdm(initial=init_epoch, total=args.num_epoch) as pbar:
        for epoch in range(init_epoch, args.num_epoch + 1):
            ...
            #                    'best_f1':best_f1}
            #         if not os.path.exists(f'datasets/{args.dataset}/seed_{args.seed}'):
            #             os.makedirs(f'datasets/{args.dataset}/seed_{args.seed}')
            #         torch.save(content, f'{args.dataset}/seed_{args.seed}/X

            if epoch % args.save_ckpt_every == 0 and epoch>100:
                with torch.no_grad():
                    X, y = generate_data(init_num, final_num, X_train, y_train, args, netG, pos_coeff, device)
                    if not os.path.exists(f'datasets/{args.dataset}/seed_{args.seed}'):
                        os.makedirs(f'datasets/{args.dataset}/seed_{args.seed}')
                    np.save(f'datasets/{args.dataset}/seed_{args.seed}/X_augmented_DGOT_epoch_{epoch}.npy',X)
                    np.save(f'datasets/{args.dataset}/seed_{args.seed}/y_augmented_DGOT_epoch_{epoch}.npy',y)
...
if __name__ == '__main__':
    ...
    np.random.seed(args.seed)
    X, y = fetch_datasets()[args.dataset]['data'], fetch_datasets()[args.dataset]['target']
    ...
    # X reshape to (size,1,feature_len)
    X_train = X_train.reshape(X_train.shape[0], 1, X_train.shape[1])
    # drop last 2 column_values
    print(f"X_train shape: {X_train.shape}, y_train shape: {y_train.shape}")
    ds = TensorDataset(torch.tensor(X_train, dtype=torch.float32), torch.tensor(y_train, dtype=torch.int32))
    args.feature_len = X_train.shape[2]
    args.class_num = len(set(y_train))
    unique_counts = np.unique(y_train, return_counts=True)
    max_count = max(unique_counts[1])
    init_num = {label: count for label, count in zip(unique_counts[0], unique_counts[1])}
    train(args, ds, init_num, max_count)