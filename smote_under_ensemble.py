...
def get_args():
    ...
    parser.add_argument("--model", type=str, default="DecisionTree",
                        choices=["DecisionTree"], help="Model to train.")
...
def main():
    ...
    if args.method == "None":
        ...
        method_params = {}
        if args.config != "None":
            with open(f'configs_ensemble/{args.config}', 'r') as f:
                config = toml.load(f)
            smote_params = config
        else:
            smote_params = None
    ...
    print(f'=========================\n')
    file_prefix = f'dataset[{args.dataset}]-model[{args.model}]-method[{args.method}]-config[{args.config}]-seed[{args.seed}]'
    ...
    with open(f"results_under_ensemble/{file_prefix}.json", "w") as f:
