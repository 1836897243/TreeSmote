...
def get_args():
    ...
    parser.add_argument("--sampler", type=str, default="None",
                        choices=["TreeSmote", "SMOTE", "KMeansSMOTE", "BorderlineSMOTE", "SVMSMOTE", "None"],
...
def get_scores(sampler, model, metric, X_train, y_train, X_test, y_test):
    ...
    if hasattr(clf, "decision_function"):
        ...
        y_score = y_pred
    scores = compute_metrics(y_test, y_score, y_pred)
    if metric == 'F1_macro':
        return scores[1]
    elif metric == 'Accuracy':
        return scores[6]
    elif metric == 'MCC_macro':
        return scores[4]
...
def main():
    ...
    if args.sampler == "TreeSmote":
        ...
        sampler = KMeansSMOTE(random_state=args.seed, **smote_params)
    ...
    with open(f"results_smote_zenodo/{file_prefix}.json", "w") as f:
