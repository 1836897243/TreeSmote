...
def to_excel(imbalance_type_, results_dict):
    ...
    for metric in ["AUPRC_macro", "F1_macro", "BAC_macro", "G_mean_macro", "MCC_macro"]:
        ...
        df.to_excel(f'excel_smote/{metric}-{imbalance_type_}-{model_}.xlsx', index=False)
...
knn = 5
results_prefix_dict = {                                                   
    "None": lambda dataset, seed : f"results_smote/{dataset}-{model_}-None-5-{seed}",
    "SVMSMOTE": lambda dataset, seed : f"results_smote/{dataset}-{model_}-SVMSMOTE-5-{seed}",
    "BorderlineSMOTE": lambda dataset, seed : f"results_smote/{dataset}-{model_}-BorderlineSMOTE-5-{seed}",
    "SMOTE": lambda dataset, seed : f"results_smote/{dataset}-{model_}-SMOTE-5-{seed}",
    "TreeSMOTE": lambda dataset, seed : f"results_smote/{dataset}-{model_}-TreeSMOTE-5-{seed}",
}   
...
for model_ in models:
    ...
    for imbalance_type_ in imbalance_types:
        # 读取Excel文件
        AUPRC_df = pd.read_excel(f'excel_smote/AUPRC_macro-{imbalance_type_}-{model_}.xlsx',keep_default_na=False)
        BAC_df = pd.read_excel(f'excel_smote/BAC_macro-{imbalance_type_}-{model_}.xlsx',keep_default_na=False)
        F1_df = pd.read_excel(f'excel_smote/F1_macro-{imbalance_type_}-{model_}.xlsx',keep_default_na=False)
        G_mean_df = pd.read_excel(f'excel_smote/G_mean_macro-{imbalance_type_}-{model_}.xlsx',keep_default_na=False)
        MCC_df = pd.read_excel(f'excel_smote/MCC_macro-{imbalance_type_}-{model_}.xlsx',keep_default_na=False)
    ...
    excel_path = f"excel_all_smote/{model_}.xlsx"
