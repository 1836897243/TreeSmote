"""SmoteRUSBoostClassifier: Random under-over-sampling integrated 
...
if not LOCAL_DEBUG:
    ...
    from ...sampler._under_sampling import RandomUnderOverSampler
...
# Properties
_method_name = "SmoteRUSBoostClassifier"
_sampler_class = RandomUnderOverSampler
...
class SmoteRUSBoostClassifier(ResampleBoostClassifier):
    ...
    sampler_ : RandomUnderOverSampler
        The base sampler.

    estimators_ : list of classifiers
        The collection of fitted sub-estimators.

    samplers_ : list of RandomUnderOverSampler
    ...
    def __init__(
        ...
        *,
        ...
        sampler = _sampler_class(k_neighbors=k_neighbors, replacement=replacement, random_state=random_state)
        sampling_type = _sampling_type

        super(SmoteRUSBoostClassifier, self).__init__(
            estimator=estimator,
        ...
        self.k_neighbors = check_type(k_neighbors, "k_neighbors", int)
    ...
    def fit(
        ...
        *,
        ...
        rus_smote_sampler_kwargs = {"replacement": self.replacement, "k_neighbors": self.k_neighbors}

        return self._fit(
            X,
            y,
            sample_weight=sample_weight,
            sampler_kwargs=rus_smote_sampler_kwargs,
...
if __name__ == "__main__":  # pragma: no cover
    ...
    rusboost = SmoteRUSBoostClassifier(**init_kwargs).fit(**fit_kwargs)
    ensembles["rusboost"] = rusboost

    init_kwargs, fit_kwargs = copy(init_kwargs_default), copy(fit_kwargs_default)
    fit_kwargs.update({"balancing_schedule": "progressive"})
    rusboost_prog = SmoteRUSBoostClassifier(**init_kwargs).fit(**fit_kwargs)
