...
class TreeSMOTE2(BaseSMOTE):
    ...
    def _fit_resample(self, X, y, sample_weight=None):
        ...
        # 2️⃣ KNN 缓存: (leaf_id, class) -> (leaf_cls_idx, X_leaf, neigh)
        knn_cache = {}
        ...
        for cls, n_gen in sampling.items():
            ...
            try_cnt = 2*n_gen
            while n_gen > 0 and try_cnt>0:
                # 随机选一个少数类样本（全局索引）
                idx_i = rng.choice(cls_idx)
                leaf = leaf_ids[idx_i]
                ...
                if key not in knn_cache:
                    # leaf 内该类别的全局索引
                    leaf_cls_idx = cls_idx[leaf_ids[cls_idx] == leaf]
                    if len(leaf_cls_idx) < 2:
                        continue  # 该叶子内该类样本不足

                    X_leaf = X[leaf_cls_idx]
                    ...
                    knn_cache[key] = (leaf_cls_idx, X_leaf, neigh)
                else:
                    leaf_cls_idx, X_leaf, neigh = knn_cache[key]

                # idx_i 在 leaf_cls_idx 中的位置（局部索引）
                pos = np.where(leaf_cls_idx == idx_i)[0]
                if len(pos) == 0:
                    continue
                pos = pos[0]

                # 邻居索引一定安全
                nn_idx = rng.choice(neigh[pos])

                # 4️⃣ 生成新样本（SMOTE 插值）
                step = rng.random()
                X_new = X_leaf[pos] + step * (X_leaf[nn_idx] - X_leaf[pos])

                X_res.append(X_new[None, :])
                y_res.append(np.array([cls]))
                n_gen -= 1
                try_cnt -=1
