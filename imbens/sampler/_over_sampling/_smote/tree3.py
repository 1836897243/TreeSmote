            ...
            while n_gen > 0:
                for leaf in leaf_ids:
                    ...
                    for i, idx in enumerate(leaf_cls_idx):
                        ...
                        X_new_sample = (X[idx] + rng.random() * (X[neighbors_idx[chosen]] - X[idx])).reshape(1, -1)
                    ...
                    if n_gen <= 0:
                        break
                    if try_cnt <= 0:
                        assert False, "Cannot generate enough samples. Try to increase k_neighbors or check the data distribution."
